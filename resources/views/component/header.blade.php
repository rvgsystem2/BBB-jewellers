...
<div class="bg-[#906224] text-sm">
  ...
  <!-- Main Header -->
  <header class="shadow-sm sticky top-0 z-50 bg-black bg-opacity-90 backdrop-blur-md">
    <div class="container mx-auto px-4 py-4 flex justify-between items-center">
      <!-- Logo -->
      <a href="/" class="text-2xl font-bold text-yellow-600 tracking-wide">
       {{-- <img src="{{asset('asset/img/logo1_236.png')}}" alt=""  class="h-16 w-90 hidden md:block"> --}}
       <img src="{{asset('asset/img/BBBJ-Frame[1].png')}}" alt="" class="h-16 w-80 ">
      </a>


      <nav class="hidden md:flex items-center space-x-8 text-sm font-semibold">
        <a href="/" class="hover:text-yellow-600 transition {{ request()->is('/') ? 'text-yellow-600' : 'text-gray-100' }}">Home</a>
        <a href="{{route('gallery')}}" class="hover:text-yellow-600 transition {{ request()->routeIs('gallery') ? 'text-yellow-600' : 'text-gray-100' }}">Gallery</a>
        <a href="{{route('product')}}" class="hover:text-yellow-600 transition {{ request()->routeIs('product') ? 'text-yellow-600' : 'text-gray-100' }}">Products</a>
        <a href="{{route('about')}}" class="hover:text-yellow-600 transition {{ request()->routeIs('about') ? 'text-yellow-600' : 'text-gray-100' }}">About Us</a>
        <a href="{{route('contact')}}" class="hover:text-yellow-600 transition {{ request()->routeIs('contact') ? 'text-yellow-600' : 'text-gray-100' }}">Contact</a>
      </nav>
...
<div class="relative w-full max-w-xs">
    ...
    <div id="searchResults" class="absolute z-50 left-0 w-full mt-1 bg-white border rounded-lg shadow-lg hidden max-h-64 overflow-y-auto">
      ...
      <div class="flex items-center gap-4">
        ...
        <a href="{{route('login')}}" class="text-gray-100 hover:text-yellow-600">
